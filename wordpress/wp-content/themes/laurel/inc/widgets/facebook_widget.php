...
add_action( 'widgets_init', 'laurel_facebook_load_widget' );

function laurel_facebook_load_widget() {
	register_widget( 'laurel_facebook_widget' );
}

class laurel_facebook_widget extends WP_Widget {

	/**
	 * Widget setup.
	 */
	function laurel_facebook_widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'laurel_facebook_widget', 'description' => __('A widget that displays a Facebook Like Box', 'laurel') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'laurel_facebook_widget' );

		/* Create the widget. */
		parent::__construct( 'laurel_facebook_widget', __('Laurel: Facebook Like Box', 'laurel'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<!-- Page url -->
		<p>
			<label for="<?php echo $this->get_field_id( 'page_url' ); ?>"><?php esc_html_e( 'Facebook Page URL', 'laurel' ); ?>:</label>
		...
		<!-- Faces -->
		<p>
			<label for="<?php echo $this->get_field_id( 'faces' ); ?>"><?php esc_html_e( 'Show Faces', 'laurel' ); ?>:</label>
		...
		<!-- Stream -->
		<p>
			<label for="<?php echo $this->get_field_id( 'stream' ); ?>"><?php esc_html_e( 'Show Stream', 'laurel' ); ?>:</label>
		...
		<!-- Cover -->
		<p>
			<label for="<?php echo $this->get_field_id( 'cover' ); ?>"><?php esc_html_e( 'Show Page Cover Image', 'laurel' ); ?>:</label>
