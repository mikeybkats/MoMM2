...
add_action( 'widgets_init', 'laurel_about_load_widget' );

function laurel_about_load_widget() {
	register_widget( 'laurel_about_widget' );
}

class laurel_about_widget extends WP_Widget {

	/**
	 * Widget setup.
	 */
	function laurel_about_widget() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'laurel_about_widget', 'description' => __('An About Me Widget', 'laurel') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 250, 'height' => 350, 'id_base' => 'laurel_about_widget' );

		/* Create the widget. */
		parent::__construct( 'laurel_about_widget', __('Laurel: About Me', 'laurel'), $widget_ops, $control_ops );
	}
	...
	function form( $instance ) {
		...
		<!-- image url -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image' ); ?>"><?php esc_html_e( 'Image URL', 'laurel' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter the image URL you want to use. You can upload your image via Media > Add New', 'laurel' ); ?></small>
		</p>
		
		<!-- image link -->
		<p>
			<label for="<?php echo $this->get_field_id( 'image_link' ); ?>"><?php esc_html_e( 'Image Link', 'laurel' ); ?>:</label>
			...
			<small><?php esc_html_e( 'Enter a link you want the about me image to go to.', 'laurel' ); ?></small>
		</p>
		
		<!-- description -->
		<p>
			<label for="<?php echo $this->get_field_id( 'description' ); ?>"><?php esc_html_e( 'About me text', 'laurel' ); ?>:</label>
		...
		<!-- autograph url -->
		<p>
			<label for="<?php echo $this->get_field_id( 'signing' ); ?>"><?php esc_html_e( 'Autograph Image URL', 'laurel' ); ?>:</label>
